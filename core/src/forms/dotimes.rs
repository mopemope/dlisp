...
pub async fn dotimes_form(
    ...
    if args.is_empty() {
        return Err("dotimes requires at least a binding form (var count)"
            .to_string()
            .into());
    }
    ...
    let binding = match &args[0] {
        Value::List(l) => l,
        _ => {
            return Err("dotimes first argument must be (var count)"
                .to_string()
                .into());
        }
    };
    ...
    if binding.len() != 2 {
        return Err("dotimes binding must be (var count)".to_string().into());
    }

    let var_name = match &binding[0] {
        Value::Symbol(s) => s.clone(),
        _ => return Err("dotimes variable must be a symbol".to_string().into()),
    };
    ...
    let count = match count_val {
        Value::Integer(n) => {
            if n < 0 {
                return Err("dotimes count must be non-negative".to_string().into());
            }
            n
        }
        _ => return Err("dotimes count must be an integer".to_string().into()),
    };
